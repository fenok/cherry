...
Since we're using SSR, we can't rely on `React.lazy`. Instead, we will use [loadable-components](https://loadable-components.com/docs/server-side-rendering/).

```bash
yarn add -D @loadable/component @loadable/server @loadable/babel-plugin @loadable/webpack-plugin @types/loadable__server @types/loadable__component
```

We also enable the `output.clean` Webpack option to clean the output directory before each build, because otherwise obsolete chunks from previous builds may remain there.

We use `webpack.optimize.LimitChunkCountPlugin` to prevent code-splitting on the server side.

We have to set the `module` TS compiler option to `"ESNEXT"` to use dynamic imports.

Note that in development we don't actually need `loadable-stats.json` generated by `LoadablePlugin`. Instead, we use `client` stats from `webpack-dev-middleware`.

### Adding production build

We use the [`env`](https://webpack.js.org/api/cli/#env) option to switch to production build. We also set `process.env.NODE_ENV` accordingly just in case. We do it in Webpack config to make sure that `webpack --env production` is all we need to make a production build.

We update `mode` and `devtool` options accordingly. We enable HMR only in development.

We follow the [Caching](https://webpack.js.org/guides/caching/) Webpack guide to enable proper caching of our assets in production mode.

We use [`DefinePlugin`](https://webpack.js.org/plugins/define-plugin/) to replace `process.env.NODE_ENV` with an appropriate value in frontend code, switching libraries like React to production mode when necessary.
